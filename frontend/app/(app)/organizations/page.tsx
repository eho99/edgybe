...
export default function OrganizationsPage() {
  ...
  const buildPayload = (mode: 'create' | 'update'): OrganizationCreatePayload | OrganizationUpdatePayload => {
    ...
    const payloadBase = {
      name,
      district_id: toOptionalString(formState.district_id),
          ...
          <CardContent className="space-y-6">
            {!isCreating && selectedOrg && restrictedRoles.has((selectedRole ?? '').toLowerCase()) && (
              <Alert>
                ...
                <AlertDescription>
                  Certain SIS and foreign key fields are hidden for staff and secretary roles.
                </AlertDescription>
              </Alert>
            )}
            ...
            {!isCreating && selectedOrg && (
              <div className="space-y-4">
                ...
                <div className="grid gap-4 md:grid-cols-2">
                  <SummaryItem label="District ID" value={selectedOrg.district_id ?? '—'} />
                  <SummaryItem label="Phone" value={selectedOrg.phone_number ?? '—'} />
                  <SummaryItem label="Address" value={[selectedOrg.street_number, selectedOrg.street_name].filter(Boolean).join(' ') || '—'} />
                  <SummaryItem
                    label="Location"
                    value={
                  ...
                  />
                  <SummaryItem label="Grades" value={
